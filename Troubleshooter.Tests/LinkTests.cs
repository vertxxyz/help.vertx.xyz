...
namespace Troubleshooter.Tests
{
	public class LinkTests
	{
		public static IEnumerable<object[]> Pages =>
			Directory.EnumerateFiles(TestUtility.TestSite.AssetsRoot, "*.md", SearchOption.AllDirectories).Select(file => new object[] {file});
		...
		public void ValidateLinks(string path)
		{
		...
		[Theory]
		[MemberData(nameof(Pages))]
		public void ValidateEmbeds(string path)
		{
			string text = File.ReadAllText(path);
			string name = Path.GetFileNameWithoutExtension(path);
			using (new AssertionScope())
