...
public sealed partial class D3
{
	...
	private static string TimeToValueGraph(IJavaScriptExecutor webDriver, Func<(float value, float deltaTime), float> map, ILogger logger)
	{
		...
		{
			List<TimeToValue> v = values[i] = new List<TimeToValue>();
			float value = origin;
			...
			} while (Math.Abs(value - destination) > 0.001f);
	...
	private static string WrongLerpGraph(IJavaScriptExecutor webDriver, ILogger logger) => TimeToValueGraph(webDriver, i => Maths.LerpUnclamped(i.value, 1, i.deltaTime * 10), logger);
	
	private static string ImprovedWrongLerpGraph(IJavaScriptExecutor webDriver, ILogger logger) => TimeToValueGraph(webDriver, i => Maths.LerpUnclamped(i.value, 1, 1 - (float)Math.Pow(0.3, i.deltaTime * 10)), logger);
}